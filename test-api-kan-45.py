...
def test_drivers_happy_path():
    setup_auth()
    response = make_request("GET", "/api/drivers", headers=get_auth_headers())
    print_req_res("GET", "/api/drivers", response)
    assert response["status"] == 200
    assert response["body"] is not None

def test_drivers_not_found():
    response = make_request("GET", "/api/drivers/nonexistent-xyz-123", headers=get_auth_headers())
    print_req_res("GET", "/api/drivers/nonexistent-xyz-123", response)
...
def test_constructors_championship_happy_path():
    setup_auth()
    response = make_request("GET", "/api/constructors-championship", headers=get_auth_headers())
    print_req_res("GET", "/api/constructors-championship", response)
    assert response["status"] == 200
    assert response["body"] is not None

def test_constructors_championship_not_found():
    response = make_request("GET", "/api/constructors-championship/nonexistent-xyz-123", headers=get_auth_headers())
    print_req_res("GET", "/api/constructors-championship/nonexistent-xyz-123", response)
    assert response["status"] in (404, 400)

def test_constructors_championship_invalid_auth():
    response = make_request("GET", "/api/constructors-championship", headers={"Authorization": "Bearer invalid-token-12345"})
    print_req_res("GET", "/api/constructors-championship", response)
    assert response["status"] in (200, 401, 403)

def test_drivers_championship_happy_path():
    setup_auth()
    response = make_request("GET", "/api/drivers-championship", headers=get_auth_headers())
    print_req_res("GET", "/api/drivers-championship", response)
    assert response["status"] == 200
    assert response["body"] is not None

def test_drivers_championship_not_found():
    response = make_request("GET", "/api/drivers-championship/nonexistent-xyz-123", headers=get_auth_headers())
    print_req_res("GET", "/api/drivers-championship/nonexistent-xyz-123", response)
    assert response["status"] in (404, 400)

def test_drivers_championship_invalid_auth():
    response = make_request("GET", "/api/drivers-championship", headers={"Authorization": "Bearer invalid-token-12345"})
    print_req_res("GET", "/api/drivers-championship", response)
    assert response["status"] in (200, 401, 403)
...
if __name__ == "__main__":
    tests = [
        ...
        test_constructors_championship_happy_path, test_constructors_championship_not_found, test_constructors_championship_invalid_auth,
        test_drivers_championship_happy_path, test_drivers_championship_not_found, test_drivers_championship_invalid_auth
    ]
    for test in tests:
        try:
            test()
